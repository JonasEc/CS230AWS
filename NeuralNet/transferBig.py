...
ap.add_argument("-m", "--model", type=str, default = path+ 'NeuralNet/my_models/SYNTHbig/last_model.h5',
...
for i in range(1,len(layer_params)):
    ...
    if i== 7:
       inner = MaxPool2D(pool_size=(2,1), strides=None, padding='valid', name=f'pool{i+1}')(inner)

inner  = MaxPool2D(pool_size=((IMGHEIGHT  // (pool_size[0] ** (len(wherePool)+1))), 1), strides=None, padding="valid")(inner)
...
for i,layer in enumerate(model.layers[:26]):
...
early_stopping= keras.callbacks.EarlyStopping(monitor='val_loss', patience=11, restore_best_weights=True, verbose=1)
